...
export class RiskAssessor {
  ...
  async detectImmediateRisk(message: string, history: RiskAssessment[]): Promise<RiskAssessment> {
    ...
    const { level, score } = this.evaluateCompositeRisk(sentiment, crisisIndicators, riskTrend);
  ...
  private async detectCrisisPatterns(message: string): Promise<CrisisIndicators> {
  ...
  private evaluateCompositeRisk(
    ...
    if (crisis.immediateAction) {
      return { level: RiskLevel.CRITICAL, score: 0.9 + crisis.severity * 0.1 };
    }
  ...
  private calculateBaseRiskScore(
    ...
    return (
      sentimentScore * weights.sentiment + crisisScore * weights.crisis + trendScore * weights.trend
    );
  }
  ...
  private determineRiskFactors(sentiment: SentimentAnalysis, crisis: CrisisIndicators): string[] {
