...
export class PlanEvolutionService {
	...
	private buildRevisionPrompt(
		...
		const recentMessages = contextUpdate.history.slice(
			-this.MAX_HISTORY_DEPTH
		)

		const userRiskProfile =
			contextUpdate.riskHistory.length > 0
				? contextUpdate.riskHistory[
						contextUpdate.riskHistory.length - 1
					]
		...
		const techniques =
			planContent?.techniques?.join(', ') || 'No specific techniques'
...
${
	recentMessages
		...
		.map(
			(msg) =>
				`- ${msg.metadata?.breakthrough || msg.metadata?.challenge}`
		)
...
return format: {
  ...
  "metrics": {
	 ...
	 */
	private async getValidatedPlanContent(
		prompt: string
	): Promise<PlanContent> {
		const revisedContent = await this.llmGateway.generateCompletion(
			prompt,
			{
		...
		try {
			...
			throw new Error(
				`Failed to parse revised plan content: ${error.message}`
			)
		}
	}
}
...
export class ContextShiftError extends Error {
	constructor(public readonly similarity: number) {
		super(
			`Significant context shift detected (similarity: ${similarity.toFixed(2)})`
		)
	}
}
